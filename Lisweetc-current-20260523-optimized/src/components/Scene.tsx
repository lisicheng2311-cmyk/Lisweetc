...
function makeGhostTextTexture(fragment: EmotionFragment, accent: string) {
  ...
  ctx.fillText(`${fragment.emotion.toUpperCase()} / GHOST ${fragment.index}`, canvas.width / 2, 402);
...
function GhostFragments({
  ...
  const group = useRef<THREE.Group>(null);
  const ghostFillOpacity = 0.032;
  const ghostFillVariance = 0.009;
  const ghostBorderOpacity = 0.16;
  const ghostBorderVariance = 0.03;
  const ghostTextOpacity = 0.29;
  ...
  useFrame(({ clock, pointer }) => {
    ...
    group.current.children.forEach((child, index) => {
      ...
      object.position.y += Math.sin(clock.elapsedTime * 0.24 + index) * 0.0014;
      object.rotation.z += Math.sin(clock.elapsedTime * 0.18 + index) * 0.0007;
    });
  });
      ...
      {ghosts.map((ghost, index) => (
        <group key={`${ghost.fragment.id}-${index}`} position={ghost.position} rotation={ghost.rotation} scale={ghost.scale} raycast={disableRaycast}>
          ...
          <lineSegments scale={[1.03, 1.04, 1]} raycast={disableRaycast}>
            ...
            <lineBasicMaterial color={ghost.accent} transparent opacity={ghostBorderOpacity + (index % 2) * ghostBorderVariance} blending={THREE.AdditiveBlending} />
          </lineSegments>
...
function ForegroundLightSheets({ progress }: { progress: number }) {
  ...
  const sheets = useMemo(
    () =>
      Array.from({ length: window.innerWidth < 700 ? 3 : 5 }).map((_, index) => ({
        color: ["#d8c7ff", "#ffd7a8", "#b7d7ff", "#f2c6ff", "#c8fff4"][index % 5],
